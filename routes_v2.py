"""API routes for multi-image and 65X support.

MVP Scope:
- LCSH and FAST vocabularies only
- Subject65X model for all 65X fields
- East Asian collection focus
"""
import uuid
import json
...
# Create router
router = APIRouter(prefix="/api", tags=["subject-heading-v2"])
...
async def build_marc650(request: MARC650Request):
    """
    Build MARC 650 fields (legacy endpoint for backwards compatibility).
    
    - **lcsh_selections**: List of LCSHMatch objects
    """
    try:
